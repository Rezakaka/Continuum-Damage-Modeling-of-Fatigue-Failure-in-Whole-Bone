...
- The model assumes small-strain anisotropic elasticity
- Damage is strain-driven and accumulates with cyclic loading
- Material properties are hard-coded using element ranges
- The UAMP requires a sensor named `NODE_TRACKED_HISTORY`
- A hard-coded debug file path exists and should be updated before use:

D:\damage\tibia\Rabbit\Damage\inps\debug_output.txt
...
This code simulates fatigue-induced stiffness degradation in bone using an Abaqus UMAT. Each element is assigned an initial modulus based on its location, damage accumulates based on local strain, and stiffness degrades progressively. The simulation automatically stops when the structure loses more than 25% of its initial stiffness.
